 ...
 */
 sendRequest(callOptions, callback) {
  ...
  request(requestOptions, (error, response, body) => {    
   this.processRequestResults(error, response, body, (processedResults, processedError) => callback(response, processedError));      
  });
}
 ...
 /**
   ...
   */
  get(callback) {
    let getCallOptions = { ...this.options, method:'GET', query:'sysparm_limit=1' };    
 ...
 */
 post(callback) {
  let getCallOptions = { ...this.options, method:'POST' };  
  this.sendRequest(getCallOptions, (results, error) => callback(results, error));
}
